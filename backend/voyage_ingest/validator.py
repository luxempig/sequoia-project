...
# Allow single-word person slugs (e.g., "rowlston") as well as hyphenated ones (e.g., "roosevelt-franklin")
...
def validate_bundle(bundle: Dict) -> List[str]:
    ...
    for i, m in enumerate(med, start=1):
        ...
        # Media link validation - warn about unsupported links but don't block
        link = (m.get("google_drive_link") or "").strip()
        if link and not _is_supported_media_link(link):
            LOG.warning(f"[{path}] media link may not be downloadable (not Drive/Dropbox): {link[:100]}")

        # Media slug validation - ensure proper format for S3 organization
        mslug = (m.get("slug") or "").strip()
        if not mslug:
            # Slug will be auto-generated by slugger.generate_media_slugs()
            pass
        elif vslug:
            # Validate format: should end with voyage_slug-NN
            if not re.search(rf"-{re.escape(vslug)}-\d{{2}}$", mslug):
                LOG.warning(f"[{path}] media slug '{mslug}' doesn't follow expected pattern ending with '-{vslug}-NN'")
