...
const DatabaseCuratorInterface: React.FC = () => {
  ...
  const deleteVoyage = async (slug: string) => {
    if (!confirm(`Are you sure you want to delete this voyage? This cannot be undone.`)) {
      return;
    }
  ...
  const duplicateVoyage = async (slug: string) => {
    const newSlug = prompt(`Enter ID for duplicated voyage:`, `${slug}-copy`);
    if (!newSlug) return;
  ...
  const unlinkMediaFromVoyage = async (mediaSlug: string, mediaTitle: string) => {
    if (!selectedVoyage) return;

    try {
      const response = await fetch(`/api/curator/media/unlink-from-voyage?media_slug=${mediaSlug}&voyage_slug=${selectedVoyage.voyage_slug}`, {
        method: 'DELETE'
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'Failed to unlink media');
      }

      // Show confirmation dialog
      setConfirmationDialog({
        type: 'success',
        title: 'Media removed from voyage',
        message: 'The media has been removed from this voyage but remains in the Media Explorer and S3 storage.',
        details: ['To permanently delete this media, go to Media Explorer.']
      });

      // Reload linked media
      await loadLinkedEntities(selectedVoyage.voyage_slug);
    } catch (error) {
      console.error('Unlink failed:', error);
      setError(`Failed to unlink media: ${error instanceof Error ? error.message : error}`);
    }
  };

  const linkMediaToVoyage = async (media: MediaItem) => {
    if (!selectedVoyage) return;
    ...
    try {
      const response = await fetch('/api/curator/media/link-to-voyage', {
        ...
        body: JSON.stringify({
          ...
          voyage_slug: selectedVoyage.voyage_slug,
          sort_order: linkedMedia.length + 1,
          notes: ''
        })
      });
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'Failed to link media');
      }

      // Show confirmation dialog
      setConfirmationDialog({
        type: 'success',
        title: 'Media added to voyage',
        message: `"${media.title || media.media_slug}" has been added to this voyage.`,
        details: [
          'You can:',
          '• Reorder media by dragging',
          '• Add voyage-specific caption',
          '• Remove from voyage (keeps in Media Explorer)'
        ]
      });
      ...
      setError(`Failed to link media: ${error instanceof Error ? error.message : error}`);
    }
  };

  const handleMediaUploadSuccess = async (mediaSlug: string) => {
    // Show confirmation dialog
    setConfirmationDialog({
      type: 'success',
      title: 'Media uploaded and added',
      message: 'Your media has been:',
      details: [
        '• Uploaded to S3 storage',
        '• Added to this voyage',
        '• Available in Media Explorer for other voyages'
      ]
    });

    // Reload linked media if we have a selected voyage
    if (selectedVoyage) {
      await loadLinkedEntities(selectedVoyage.voyage_slug);
    }
  };
    ...
    <Layout>
      <div className="min-h-screen bg-gray-100 p-4">
                  ...
                  <div className="space-y-6">
                    <div className="bg-white rounded-lg shadow-lg p-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="text-sm font-medium text-gray-700">Voyage ID</label>
                    ...
                    {/* Linked People Panel - Split into Crew and Passengers */}
                    <div className="bg-white rounded-lg shadow-lg p-6">
                      <h3 className="text-lg font-bold text-gray-800 mb-4">People ({linkedPeople.length})</h3>
                      ...
                      {linkedPeople.length === 0 ? (
                        ...
                        <div className="space-y-6">
                          {/* Crew Section */}
                          {(() => {
                            const crew = linkedPeople.filter(p =>
                              p.capacity_role?.toLowerCase().includes('crew') ||
                              p.capacity_role?.toLowerCase().includes('captain') ||
                              p.capacity_role?.toLowerCase().includes('officer') ||
                              p.role_title?.toLowerCase().includes('crew') ||
                              p.role_title?.toLowerCase().includes('captain') ||
                              p.role_title?.toLowerCase().includes('officer')
                            );
                            return crew.length > 0 ? (
                              <div>
                                ...
                                <div className="space-y-2">
                                  {crew.map((person) => (
                                    <div key={person.person_slug} className="border rounded p-3 bg-blue-50">
                          ...
                          {(() => {
                            const passengers = linkedPeople.filter(p =>
                              !(p.capacity_role?.toLowerCase().includes('crew') ||
                                p.capacity_role?.toLowerCase().includes('captain') ||
                                p.capacity_role?.toLowerCase().includes('officer') ||
                                p.role_title?.toLowerCase().includes('crew') ||
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-6">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold text-gray-800">Media ({linkedMedia.length})</h3>
                      ...
                      {linkedMedia.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {linkedMedia.map((media) => (
                            <div key={media.media_slug} className="border rounded p-3 flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                {media.credit && (
                                  <p className="text-xs text-gray-500 mt-1">Credit: {media.credit}</p>
                                )}
                              </div>
                              <button
                                onClick={() => unlinkMediaFromVoyage(media.media_slug, media.title || media.media_slug)}
                                className="ml-3 text-red-600 hover:text-red-800 text-sm"
                                title="Remove from voyage (keeps in Media Explorer)"
                              >
        ...
        <MediaSearchModal
          ...
          onSelect={(media) => {
            linkMediaToVoyage(media);
            setShowMediaSearch(false);
          }}
          excludeMediaSlugs={linkedMedia.map(m => m.media_slug)}
        />
        ...
        <MediaUploadDialog
          ...
          voyageSlug={selectedVoyage?.voyage_slug}
        ...
        <div className="space-y-4">
          {!isNew && (
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Auto-generated from president and date</p>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded-lg p-4 bg-yellow-50">
            ...
            <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
              <li>Voyage ID is auto-generated from president and date</li>
